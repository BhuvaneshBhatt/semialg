...
def _semialgebraic_image_data(
    ...
    *,
    ...
    if image_variables is None:
        targets = collision_free_real_symbols("y", len(maps), condition, maps, source, params)
    else:
...
def semialgebraic_preimage(
    ...
    *,
    ...
    if len(set(target_vars)) != len(target_vars):
...
def _distance_via_smooth_algebraic_variety(
    ...
    from .region_analysis import _jacobian_rank_singular_result
    ...
    if not equalities or inequalities or disequalities:
        return None
    # This specialization is limited to pure algebraic varieties.
    if any(
        not isinstance(atom, sp.Equality)
        for atom in (formula.args if isinstance(formula, sp.And) else (formula,))
    ):
        return None
    singular = _jacobian_rank_singular_result(equalities, variables).require_complete()
    ...
    try:
        solutions = solve_zero_dimensional_system(
            (*equalities, *gradient_equations), variables=solve_vars, real=True
        )
    except (TypeError, ValueError, ArithmeticError, NotImplementedError):
        return None
    if not solutions:
...
def path_between(
    ...
    this is not mislabeled as a full Canny-style roadmap path.
    """
...
@dataclass(frozen=True)
class CriticalValueImage:
    """Exact image information for the critical locus of an objective."""

    value_symbol: sp.Symbol
    isolated_values: tuple[sp.Expr, ...]
    component_images: tuple[sp.Expr, ...]
    formula: sp.Expr


def critical_value_image(
    expression, region=sp.true, variables=None, *, value_symbol: sp.Symbol | str | None = None
) -> CriticalValueImage:
    """Return isolated values and exact images of positive-dimensional critical loci.

    Unlike :func:`critical_values`, this operation does not discard a critical
    component merely because the objective varies on it.  Each supported KKT or
    singular component is sent through the ordinary exact ``function_range``
    machinery and the resulting value sets are unioned with isolated values.
    """
    ...
    condition = normalize_formula(region)
    vars_ = normalize_problem_variables(variables, sp.Tuple(expr, condition))
    value = resolve_symbol(value_symbol or "t")
    isolated = critical_values(expr, condition, vars_)
    equalities, inequalities, _ = _constraint_data(condition, vars_)
    images: list[sp.Expr] = []
    for locus in iter_positive_dimensional_critical_loci(
        expr, condition, vars_, equalities, inequalities
    ):
        locus_formula = sp.And(
            condition, *(sp.Eq(equation, 0) for equation in locus.equations), evaluate=False
        )
        try:
            image = sp.sympify(
                function_range(expr, locus_formula, vars_, value_symbol=value, method="qe")
            )
        except (ArithmeticError, TypeError, ValueError, NotImplementedError, sp.PolynomialError):
            continue
        if image is not sp.false and image != sp.false and image not in images:
            images.append(image)
    pieces = [*(sp.Eq(value, item) for item in isolated), *images]
    formula = sp.simplify(sp.Or(*pieces)) if pieces else sp.false
    return CriticalValueImage(value, isolated, tuple(images), formula)
