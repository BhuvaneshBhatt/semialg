...
def _ordered_free_parameters(
    ...
    if parameters is not None:
        normalized = _normalize_symbols(parameters, expr=expr)
        undeclared = expr.free_symbols - set(variables) - set(normalized)
        if undeclared:
            names = ", ".join(
                sp.sstr(symbol) for symbol in sorted(undeclared, key=symbol_identity_key)
            )
            raise ValueError(f"undeclared symbolic parameters: {names}")
        return normalized
    vset = set(variables)
