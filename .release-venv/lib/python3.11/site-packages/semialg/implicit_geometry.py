...
class ImplicitFormulaPiece:
    ...
    Unequalities are ignored because they do not change ordinary
...
class VerticalBoundCell2D:
    ...
    def is_full_dimensional(self) -> bool:
        ...
        return certified_pointwise_zero(hi - lo) is False

    @property
    def dimension(self) -> int:
        if not self.is_full_dimensional:
            return 1 if self.y_bounds else 0
        if all(certified_equal(upper, lower) is True for lower, upper in self.y_bounds):
            return 1
        return 2
...
def semialgebraic_level_function(
    ...
    Equalities are unsupported because a single ``f <= 0`` level
...
def decompose_cylindrical_formula_to_vertical_bounds_2d(
    ...
    This is a descriptive, public helper for a 2D CAD stack
    ...
    for disjunct in _as_disjuncts(expr):
        ...
        if equal:
            if len(equal) > 1 and any(
                certified_equal(item, equal[0]) is not True for item in equal[1:]
            ):
                continue
