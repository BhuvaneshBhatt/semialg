...
def _domain_set_from_condition(condition: sp.Expr, variable: sp.Symbol) -> sp.Set | None:
    ...
    for atom in atoms:
        ...
        if sign == 0:
            truth = _relation_from_scalar(const, op)
            if truth is None:
                return None
            if not truth:
                return sp.S.EmptySet
        ...
        if op in {">", ">="}:
            if lower == -sp.oo or _finite_compare(lower, bound) < 0:
                lower = bound
                left_open = op == ">"
            elif certified_equal(lower, bound) is True:
                left_open = left_open or op == ">"
        elif op in {"<", "<="}:
            if upper == sp.oo or _finite_compare(bound, upper) < 0:
                upper = bound
                right_open = op == "<"
            elif certified_equal(upper, bound) is True:
...
def _relation_from_scalar(value: sp.Expr, op: str) -> bool | None:
    if op == "<":
        return bool(value < 0)
    if op == "<=":
        return bool(value <= 0)
    ...
    if op == ">=":
        return bool(value >= 0)
    if op == "==":
        return certified_pointwise_zero(value)
    if op == "!=":
        zero = certified_pointwise_zero(value)
