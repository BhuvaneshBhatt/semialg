...
from ._zero_testing import certified_constant_sign, certified_zero
    ...
    if len(equalities) == 1 and equalities[0].degree == 2:
        ...
        if inertia is not None and (inertia[0] == len(variables) or inertia[1] == len(variables)):
            try:
                ...
                if (
                    len(critical) == 1
                    and certified_zero(equalities[0].residual.subs(critical[0])) is True
                ):
                    return True
...
def _topology_negative_certificate(
    ...
    *,
    ...
    Full connectivity reconstruction is reserved for algebraic
