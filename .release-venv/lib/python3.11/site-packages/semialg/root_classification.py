...
def _multiplicity_pattern(poly: sp.Poly) -> tuple[int, ...]:
    ...
    for root in roots:
        for index, (known, multiplicity) in enumerate(groups):
            if certified_equal(root, known) is True:
...
def _unparameterized(expr: sp.Expr, variable: sp.Symbol) -> RootClassificationResult:
    ...
    count = sp.oo if poly.is_zero else sp.Integer(len(pattern))
    cell = RootClassificationCell(sp.true, count, pattern, sample={})
...
def _sample_for(
    ...
    if condition is sp.false or condition is False:
        return None
    return sample_point(condition, parameters, strategy="representative", strict=False)
...
def _quadratic_family(
    ...
    *,
    ...
    if certified_equal(leading, 1) is True:
        conditions = [
...
def _quartic_family(
    ...
    and is left as an unknown-count stratum here. Leading-
...
def _sampled_parameter_cells(
    ...
    This is conservative: discriminant cells are useful diagnostics,
