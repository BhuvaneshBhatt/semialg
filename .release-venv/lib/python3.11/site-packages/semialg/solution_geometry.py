...
class SolutionPlotData:
    ...
    The object avoids depending on plotting libraries. Points,
    ...
    if len(variables) == 2 and cells:
        ...
        for cell in cells:
            ...
            for lower, upper in y_bounds:
                ...
                if all(
                    certified_equal(upper_curve[i][1], lower_curve[i][1]) is True for i in range(n)
                ):
...
def discretize_region_geometry(
    ...
    in :mod:`semialg.standard_regions`. It is lightweight: it
