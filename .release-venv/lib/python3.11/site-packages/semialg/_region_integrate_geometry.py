...
def _radial_radii_squared(
    ...
    for atom in atoms:
        ...
        if certified_zero(coeff_x2) is True or certified_equal(coeff_x2, coeff_y2) is not True:
            return None
...
def _proportional_linear_form(signature, target):
    """Return the certified nonzero scale relating two linear coefficient tuples."""
    pivot = next((i for i, value in enumerate(target) if value != 0), None)
    if pivot is None or certified_zero(signature[pivot]) is True:
        return None
    scale = sp.simplify(signature[pivot] / target[pivot])
    if certified_sign(scale) not in (-1, 1):
        return None
    if all(
        certified_equal(value, scale * expected) is True
        for value, expected in zip(signature, target, strict=True)
    ):
        return scale
    return None


def _relation_matches_scaled_form(signature, op, target, target_direction):
    scale = _proportional_linear_form(signature, target)
    if scale is None:
        return False
    sign = certified_sign(scale)
    direction = op
    if sign == -1:
        direction = {"<": ">", "<=": ">=", ">": "<", ">=": "<="}[op]
    return direction in target_direction
...
def _reduce_standard_simplex_2d(
    ...
    """Recognize algebraically equivalent presentations of the 2D unit simplex."""
    ...
    targets = (
        ("x_nonnegative", (sp.Integer(1), sp.Integer(0), sp.Integer(0)), (">", ">=")),
        ("y_nonnegative", (sp.Integer(0), sp.Integer(1), sp.Integer(0)), (">", ">=")),
        ("sum_le_one", (sp.Integer(1), sp.Integer(1), sp.Integer(-1)), ("<", "<=")),
    )
    for atom in atoms:
        ...
        if linear is None:
            return None
        signature = (*linear[0], linear[1])
        matches = [
            name
            for name, target, directions in targets
            if _relation_matches_scaled_form(signature, op, target, directions)
        ]
        if len(matches) != 1:
            return None
        needed[matches[0]] = True
...
def _axis_aligned_ellipse_data(
    ...
    try:
        ...
        return None
    if poly.total_degree() != 2 or certified_zero(poly.coeff_monomial(x * y)) is not True:
        return None
