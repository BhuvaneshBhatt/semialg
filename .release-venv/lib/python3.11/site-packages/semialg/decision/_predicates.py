            ...
            if linear_profile.linear and linear_profile.conjunctive:
                ...
                if linear is not None:
                    ...
                    if sat:
                        ...
                        if witness is None and linear_presolve.changed:
                            reduced_witness = _find_validated_witness(
                                linear_presolve.formula,
                            ...
                            if reduced_witness is not None:
                                ...
                                for variable, replacement in reversed(
                                    linear_presolve.substitutions
                                ):
                                    reconstructed[variable] = sp.simplify(
                                        replacement.subs(reconstructed)
                                    )
                                if all(variable in reconstructed for variable in vars_):
                                    candidate = {
                                        variable: reconstructed[variable] for variable in vars_
                                    }
                                    if sp.simplify(expr.subs(candidate)) is sp.true:
                                        witness = candidate
        if sat is None:
            ...
            if rur_result is not None and not rur_result.partial:
                ...
                # Positive-dimensional pure equality systems are handed to the
                # certified ARS polar-reduction backend before any general CAD.
                from ..real_algebraic import real_algebraic_formula_feasibility

                ars = real_algebraic_formula_feasibility(expr, vars_)
                if ars is not None and ars.complete and ars.satisfiable is not None:
                    sat = bool(ars.satisfiable)
                    method = ars.method
                    witness = ars.assignment
                if sat is None:
                    # A validated witness can certify feasibility cheaply; failure to
                    # find one carries no information about unsatisfiability.
                    witness = _find_validated_witness(expr, vars_, strategy=strategy)
                if sat is None and witness is not None:
                    sat = True
                    ...
                    if sat is None:
                        # Existential satisfiability needs only one true branch, while
                        # proving UNSAT requires exhaustive traversal. The lazy CAD
                        # engine has exactly those semantics and avoids materializing
                        # the unused branches of a complete CAD.
                        from ..partial.qe import lazy_find_inst_form

                        lazy = lazy_find_inst_form(vars_, parse_formula(expr))
                        sat = bool(lazy.found)
                        witness = lazy.instance
                        method = "partial_cad_lazy"
