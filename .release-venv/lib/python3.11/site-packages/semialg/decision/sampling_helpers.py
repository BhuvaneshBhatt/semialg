...
def _normalize_sample_request(
    ...
    key = {"cells": "per_cell", "components": "per_component"}.get(key, key)
...
def _collect_structural_samples(
    ...
    *,
    ...
    if structural and mode in {"per_component", "per_cell"}:
        return structural
    if count <= 0:
        return ()
    if len(structural) >= count:
        return structural[:count]

    needed = count - len(structural)
    generated = tuple(
        point
        for point in sample_points(formula, variables, count=needed, strategy=strategy or "auto")
    ...
    if not generated:
        generated = tuple(
            ...
            for point in sample_points(
                original_formula, variables, count=needed, strategy=strategy or "auto"
            )
            if satisfies_formula(original_formula, point, strict=False)
        )
    return _dedupe_samples((*structural, *generated))[:count]
