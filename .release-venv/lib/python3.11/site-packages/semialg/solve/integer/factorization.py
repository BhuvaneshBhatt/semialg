...
def _prune_branch_cons(
    ...
    try:
        ...
        return None
    if equals:
        if any(certified_zero(factor.subs(var, e)) is True for e in equals):
            return None
...
def prune_divisor_assign(
    ...
    for factor, value in zip(symbolic_factors, assigned_values, strict=True):
        ...
        if equals:
            if not any(certified_equal(factor.subs(var, e), value) is True for e in equals):
                return "fixed_value_conflict"
        if lower is not None and upper is not None and lower == upper:
            if certified_zero(factor.subs(var, lower) - value) is not True:
