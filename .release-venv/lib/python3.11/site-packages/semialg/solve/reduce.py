...
def _reduce_reals(parsed: ParsedPrenexFormula, config=None, *, strategy: str | None = None):
    ...
    if strategy_name in {
        ...
        # ARS is a sentence-level feasibility backend: use it when every
        # quantified variable is existential and the matrix is a conjunction
        # of polynomial equalities with no remaining free symbols.
        if strategy_name in {"auto", "planner", "ars"} and parsed.quantifiers:
            quantified = {sym for q, sym in parsed.quantifiers if str(q).lower() == "exists"}
            all_existential = len(quantified) == len(parsed.quantifiers)
            matrix_symbols = set(getattr(parsed.matrix_expr, "free_symbols", set()))
            if all_existential and matrix_symbols <= quantified:
                from ..real_algebraic import real_algebraic_formula_feasibility

                ars = real_algebraic_formula_feasibility(parsed.matrix_expr, tuple(parsed.vars))
                if ars is not None and ars.complete and ars.satisfiable is not None:
                    return (sp.true if ars.satisfiable else sp.false), ars, selection
            if strategy_name == "ars":
                raise NotImplementedError("formula is outside the certified ARS equality fragment")
...
def reduce_formula(
    ...
    *,
    ...
    if dom is SolveDomain.REALS:
        ...
        backend_name = getattr(qe_result, "backend", "")
        specialized_method = getattr(qe_result, "method", "")
        if specialized_method.startswith("ars"):
            method = specialized_method
        elif backend_name == "quadratic-virtual-substitution-qe":
            method = "quadratic_virtual_substitution"
        elif backend_name == "rational-univariate-formula-solver":
            method = "rational_univariate"
        else:
            method = "cad"
        if selection is not None:
            metadata["strategy_selection"] = selection
        elif method == "cad" and (strategy or "collins").lower() in {"auto", "planner"}:
            metadata["strategy_selection"] = _safe_strategy_selection(parsed, parsed.vars)
        if method not in {"rational_univariate"} and not method.startswith("ars"):
