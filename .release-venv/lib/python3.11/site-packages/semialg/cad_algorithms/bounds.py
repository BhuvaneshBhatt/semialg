...
class CertifiedRootComparison:
    ...
    that semialg declined to infer a global ordering.
    """
...
def _root_function_expr(root, candidate_solver) -> sp.Expr:
    ...
    # A defining factor with one real root may occupy a later position in the
    # full lifting stack.  Its family-local root index is nevertheless zero.
    if len(candidates) == 1:
        return candidates[0]
...
class AlgebraicRootFunction(CADBound):
    ...
    def evaluate(self, base_point: Mapping[sp.Symbol, object] | None = None) -> Sample:
        ...
        try:
            ...
            if 0 <= self.root_index < len(values):
                ...
                for root in roots:
                    if certified_equal(root.as_expr(), value) is True:
    ...
    def derivative_expr(self, variable: sp.Symbol, *, require_regular: bool = True) -> sp.Expr:
        ...
        if require_regular and certified_zero(denominator) is True:
    ...
    def specialize(self, base_point: Mapping[sp.Symbol, object]) -> CADBound:
        ...
        specialization returns a new root function and drops the
...
def verify_cad_cell_bounds(cell: object) -> CADCellBoundsCertificate:
    ...
    The verifier is local and auditable: it checks triangular
    ...
    for i, level in enumerate(levels):
        ...
        if getattr(level, "is_section", False):
            section_verified = certified_equal(lower_expr, upper_expr) is True
            ...
            if getattr(display.func, "__name__", "") != "root_of":
                try:
                    ...
                    if cert.sample_root_value is None:
                        ...
                        repr_consistent = (
                            repr_consistent
                            and certified_equal(displayed_value, cert.sample_root_value) is True
                        )
        ...
        try:
            ...
            if getattr(level, "is_section", False):
                sample_contained = certified_equal(value, lo) is True
            else:
                if lo == -sp.oo:
                    left_ok = True
                else:
