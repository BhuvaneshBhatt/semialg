...
from .._zero_testing import certified_equal, certified_pointwise_zero, certified_zero
...
class CADBoundaryDescriptor:
    ...
    APIs.  Explicit non-root boundaries return no descriptor.
    """
...
def _section_delineability_certificate(
    ...
    *,
    ...
    sample_root_verified = certified_zero(poly.subs(sample_subs)) is True
    local_root_index = 0
    try:
        ...
        matches = [i for i, root in enumerate(roots) if certified_equal(root, sample_value) is True]
        if matches:
    ...
    try:
        ...
        for idx, candidate in enumerate(candidates):
            ...
            if certified_equal(specialized_candidate, sample_value) is True:
                matches.append(idx)
        if len(matches) == 1:
    ...
    try:
        ...
        sample_deriv_nonzero = certified_pointwise_zero(fiber_derivative) is False
...
def _level_condition(
    ...
    if kind == "section" or certified_equal(upper, lower) is True:
