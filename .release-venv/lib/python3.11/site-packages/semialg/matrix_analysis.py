...
from ._zero_testing import certified_constant_sign, certified_equal
...
_exact_constant_sign = certified_constant_sign
...
def constant_symmetric_inertia(matrix: sp.MatrixBase) -> tuple[int, int, int] | None:
    ...
    for row in range(immutable_matrix.rows):
        for column in range(row + 1, immutable_matrix.cols):
            if (
                certified_equal(immutable_matrix[row, column], immutable_matrix[column, row])
                is not True
            ):
                return None
...
def matrix_definiteness(
    ...
    *,
    ...
    for row in range(immutable_matrix.rows):
        for column in range(row + 1, immutable_matrix.cols):
            equality = certified_equal(immutable_matrix[row, column], immutable_matrix[column, row])
            if equality is not True:
                raise ValueError("matrix definiteness requires a certifiably symmetric matrix")
