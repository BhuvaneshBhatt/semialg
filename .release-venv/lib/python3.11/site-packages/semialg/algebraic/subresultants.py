...
from .._flint_arithmetic import flint_resultant
from .modular import modular_resultant_qq, modular_subresultants_qq
...
def subresultant_prs(
    ...
    *,
    ...
    primitive:
        ...
        conventions.
    modular:
        Attempt certified modular reconstruction over ``QQ`` first. Unsupported
        coefficient domains and inconclusive reconstruction fall back to the
        ordinary exact implementation.
    """

    var = _infer_variable(first, second, variable)
    if modular and domain in (None, sp.QQ):
        accelerated = modular_subresultants_qq(first, second, var)
        if accelerated is not None:
            try:
                polys = tuple(
                    _as_poly(expr, var, domain=domain) for expr in accelerated.polynomials
                )
            except _RECOVERABLE_ERRORS:
                polys = tuple(_as_poly(expr, var) for expr in accelerated.polynomials)
            if primitive:
            ...
            return SubresultantPRSResult(
                ...
                resultant=sp.factor(accelerated.resultant),
                source=accelerated.certificate.method,
            )
    ...
    if primitive:
        polys = tuple(_primitive_part(poly) for poly in polys)
    accelerated_resultant = (
        modular_resultant_qq(left.as_expr(), right.as_expr(), var) if modular else None
    )
    if accelerated_resultant is not None:
        resultant = sp.factor(accelerated_resultant.resultant)
    else:
        flint_value = flint_resultant(left, right, var)
        resultant = sp.factor(
            flint_value
            if flint_value is not None
            else sp.resultant(left.as_expr(), right.as_expr(), var)
        )
    return SubresultantPRSResult(
        variable=var,
