...
def _apply_substitutions_to_constraints(
    ...
    for atom in atoms:
        if isinstance(atom, sp.Equality):
            if (
                atom.lhs in substitutions
                and certified_equal(atom.rhs, substitutions[atom.lhs]) is True
            ):
                if keep_definitions:
                    out.append(atom)
                continue
            if (
                atom.rhs in substitutions
                and certified_equal(atom.lhs, substitutions[atom.rhs]) is True
            ):
                if keep_definitions:
                    out.append(atom)
                continue
...
def simplify_system(
    ...
    *,
    ...
    reducer. It returns a mathematically equivalent formula rather
