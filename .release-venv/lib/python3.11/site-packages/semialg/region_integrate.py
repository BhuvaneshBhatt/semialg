...
def integrate_over_region(
    ...
    *,
    ...
    if isinstance(condition, StandardRegion):
        ...
        if measure_dimension not in (None, "ambient", "intrinsic", "top"):
            try:
                natural_dimension = condition.dimension()
            except NotImplementedError:
                natural_dimension = None
            if natural_dimension is None or measure_dimension != natural_dimension:
                raise NotImplementedError(
    ...
    bound_map = normalize_bounds(bounds, vars_)
    bounded_formula = _formula_with_explicit_bounds(formula, bound_map)
    dim = _normalize_measure_dimension(measure_dimension, vars_, bounded_formula)
    ...
    if dim != len(vars_):
        ...
        value, intrinsic_method = _integrate_intrinsic_dimension(
            expr, bounded_formula, vars_, dim, bounds=bound_map
        )
        result = RegionIntegralResult(
            value=value,
            integrand=expr,
