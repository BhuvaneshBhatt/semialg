...
Represent polygon holes and polyhedral shells explicitly, then query outer and inner boundary components without reconstructing them numerically.
...
The polygon example has one outer boundary and one hole. The cube has one outer shell and no cavity shells.
