...
def _root_function_expr(root, candidate_solver) -> sp.Expr:
    """Present a certified algebraic root function with an explicit radical solver."""

    from ..reconstruct.root_functions import primitive_fiber_polynomial, root_of

    poly = primitive_fiber_polynomial(root.polynomial, root.fiber_variable)
    cert = root.certificate
    if cert is not None and cert.representation_verified and cert.radical_branch_index is not None:
        candidates = candidate_solver(poly, root.fiber_variable, ordered=False)
        if 0 <= cert.radical_branch_index < len(candidates):
            return candidates[cert.radical_branch_index]

    # Linear roots and quadratics with a fixed-sign leading coefficient have a
    # globally ordered radical presentation wherever their real roots exist.
    # Using that exact presentation keeps reconstructed formulas polynomial or
    # radical-semialgebraic instead of introducing an opaque root_of node.
    candidates = candidate_solver(poly, root.fiber_variable, ordered=True)
    if 0 <= root.root_index < len(candidates):
        return candidates[root.root_index]

    if poly.free_symbols <= {root.fiber_variable}:
        try:
            roots = tuple(sp.real_roots(poly))
            if 0 <= root.root_index < len(roots):
                return roots[root.root_index]
        except (sp.PolynomialError, ValueError, TypeError, NotImplementedError):
            pass
    return root_of(poly, root.fiber_variable, sp.Integer(root.root_index))
...
class AlgebraicRootFunction(CADBound):
    ...
    def as_expr(self) -> sp.Expr:
        ...
        return _root_function_expr(self, fiber_root_candidates)

    def evaluate(self, base_point: Mapping[sp.Symbol, object] | None = None) -> Sample:
        """Evaluate this algebraic root function at an exact point of its CAD base cell."""
        ...
        from ..reconstruct.root_functions import primitive_fiber_polynomial

        primitive = primitive_fiber_polynomial(self.polynomial, self.fiber_variable)
        specialized = sp.expand(primitive.subs(substitutions))
        ...
        try:
            roots = isolate_real_roots(
                exact_univariate_poly(specialized, self.fiber_variable, algebraic_extension=False)
            )
    ...
    def specialize(self, base_point: Mapping[sp.Symbol, object]) -> CADBound:
        ...
        source cell certificate, because its domain certificate belongs to the
