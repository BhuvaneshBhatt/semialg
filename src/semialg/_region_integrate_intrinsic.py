...
from ._zero_testing import certified_equal, certified_pointwise_zero
...
def _zero_dimensional_points(
    ...
    for atom in atoms:
        if isinstance(atom, Equality):
            ...
            if certified_equal(expr, 0) is not True:
...
def _integrate_zero_dimensional(
    ...
    if condition is sp.false or condition == sp.false:
        return sp.Integer(0)
    total = sp.Integer(0)
...
def _circle_radius_squared(condition: sp.Expr, x: sp.Symbol, y: sp.Symbol) -> sp.Expr | None:
    ...
    for atom in atoms:
        if isinstance(atom, Equality):
            ...
            if (
                certified_pointwise_zero(coeff_x2) is False
                and certified_equal(coeff_x2, coeff_y2) is True
                ...
                if radius_sq is None:
                    radius_sq = candidate
                elif certified_equal(radius_sq, candidate) is not True:
                    return None
            else:
    ...
    if radius_sq is None or other_atoms:
        return None
    if radius_sq.is_negative is True:
        return sp.Integer(0)
...
def _graph_curve_data(
    ...
    for atom in atoms:
        if isinstance(atom, Equality):
            ...
            if certified_equal(graph, candidate) is not True:
                return None
...
def _integrate_affine_intrinsic_chart(
    integrand: sp.Expr,
    condition: sp.Expr,
    variables: Sequence[sp.Symbol],
    measure_dimension: int,
) -> sp.Expr | None:
    """Integrate an equality-defined affine region in intrinsic coordinates."""
    branches = sp.Or.make_args(condition)
    branch_rows: list[sp.Matrix] = []
    for branch in branches:
        rows = []
        for atom in sp.And.make_args(branch):
            if not isinstance(atom, Equality):
                continue
            expr, _ = _relation_parts(atom)
            try:
            ...
            if poly.total_degree() <= 1:
                rows.append([poly.coeff_monomial(v) for v in variables] + [poly.coeff_monomial(1)])
        if not rows:
            return None
    ...
    if len(solution) != 1:
        return None
    mapping_values = tuple(next(iter(solution)))
    free = tuple(var for var in variables if any(value.has(var) for value in mapping_values))
    if len(free) != measure_dimension:
        return None
    mapping = dict(zip(variables, mapping_values, strict=True))
    pulled_branches = []
    for branch in branches:
        atoms = []
        ...
        impossible = False
        for atom in sp.And.make_args(branch):
            value = sp.simplify(atom.xreplace(mapping))
            ...
            if isinstance(value, Equality):
                ...
                break
            atoms.append(value)
        if not impossible and not lower_dimensional:
            pulled_branches.append(sp.And(*atoms))
    if not pulled_branches:
        return sp.Integer(0)
    pulled_condition = sp.Or(*pulled_branches)
    pulled_integrand = sp.simplify(integrand.xreplace(mapping))
    jacobian = sp.Matrix(mapping_values).jacobian(free)
    metric = sp.sqrt(sp.simplify((jacobian.T * jacobian).det()))
    from .region_integrate import integrate_over_region

    return sp.simplify(
        integrate_over_region(
            sp.simplify(pulled_integrand * metric),
            pulled_condition,
            free,
            method="symbolic",
...
def _integrate_intrinsic_dimension(
    ...
    *,
    ...
    if condition is sp.false or condition == sp.false or measure_dimension < 0:
        return sp.Integer(0), "empty_region_zero_measure"
    ...
    affine = _integrate_affine_intrinsic_chart(integrand, condition, variables, measure_dimension)
    if affine is not None:
        return affine, "affine_intrinsic_parametric_measure"
    ...
    try:
        ...
        selected_cells = tuple(cell for cell in solution.cells if cell.selected)
        if not selected_cells:
            return sp.Integer(0), "empty_cylindrical_solution_zero_measure"
