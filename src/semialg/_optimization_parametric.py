    ...
    for atom in atoms:
        ...
        if isinstance(atom, (sp.LessThan, sp.StrictLessThan)):
            if atom.lhs == variable and variable not in atom.rhs.free_symbols:
                ...
                if upper is not None and sp.simplify(upper[0] - candidate[0]) != 0:
                    return None
                upper = candidate
                continue
            if atom.rhs == variable and variable not in atom.lhs.free_symbols:
                candidate = (atom.lhs, strict)
                if lower is not None and sp.simplify(lower[0] - candidate[0]) != 0:
                    return None
                lower = candidate
                continue
...
def _eliminate_parametric_relation(
    ...
    Parametric optimization and range APIs deliberately keep their exact
