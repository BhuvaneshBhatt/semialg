...
def region_intersection(*regions: FormulaLike | Iterable[FormulaLike]):
    """Return the exact intersection of semialgebraic regions.

    Canonical region pairs are simplified structurally when a supported exact
    representation is available.  General inputs lower to the unified symbolic
    representation without changing the represented set.
    """
    if len(regions) == 2:
        from ..region_structural import UNRESOLVED, structural_intersection

        structural = structural_intersection(regions[0], regions[1])
...
def region_product(*regions, variables=None):
    ...
    Products of closed intervals/boxes and products of points retain canonical
    structure.  Other products are represented by a ``SemialgebraicRegion`` in
    concatenated coordinates.
    """
    if not regions:
        raise ValueError("region_product requires at least one region")
    from ..region_structural import UNRESOLVED, structural_product
    from ..symbolic_regions import SemialgebraicRegion, as_semialgebraic_region

    result = regions[0]
    for region in regions[1:]:
        structural = structural_product(result, region)
        if structural is not UNRESOLVED:
            result = structural
            continue
        left = as_semialgebraic_region(result)
        right = as_semialgebraic_region(region)
        total = left.ambient_dimension + right.ambient_dimension
        if variables is None:
            coords = tuple(sp.Symbol(f"x{i}", real=True) for i in range(total))
        else:
            coords = tuple(sp.Symbol(v, real=True) if isinstance(v, str) else v for v in variables)
            if len(coords) != total:
                raise ValueError("variables must match the product ambient dimension")
        left_coords = coords[: left.ambient_dimension]
        right_coords = coords[left.ambient_dimension :]
        formula = sp.And(
            left.membership_formula(left_coords),
            right.membership_formula(right_coords),
        )
        result = SemialgebraicRegion(formula, coords)
    return result
...
def _cad_region_dimension(expr: sp.Expr, variables: Sequence[sp.Symbol]) -> int:
    """Return exact semialgebraic dimension directly from selected CAD leaves.

    Dimension depends only on which CAD leaves satisfy the formula and on each
    leaf's native section/sector structure. It does not require reconstructing
    algebraic section values or a symbolic formula for the selected cell union.
    """
    ...
    selected_cells = _select_formula_cells(cad_obj, formula, variables)
    if not selected_cells:
        return -1
    kinds = {
        cell.index: cell.kind
        for level_cells in cad_obj.cells_by_level.values()
        for cell in level_cells
    }
    return max(
        sum(kinds[cell.index[:level]] == "sector" for level in range(1, len(variables) + 1))
