...
class SemialgebraicRegion(sp.Basic):
    ...
    __slots__ = ("_context", "_cad", "_qf_formula", "_native_analysis")
    ...
    def ensure_native_analysis(self):
        """Build native selected-cell state once for invariant queries."""
        if self._native_analysis is None:
            from .region_native_analysis import NativeRegionAnalysis

            self._native_analysis = NativeRegionAnalysis.build(
                self.quantifier_free_formula(), self.variables
            )
        return self._native_analysis
    ...
    def product(self, other: object, *, variables=None):
    ...
    def image(
        ...
        *,
        ...
        targets = (
            ...
            else collision_free_real_symbols(
                "y", len(maps), self.formula, self.variables, maps, start=1
            )
        )
    ...
    def euler_characteristic(self, *, compact_support: bool = True) -> int:
        ...
        return self.ensure_native_analysis().euler_characteristic()
    ...
    def intrinsic_parametric_cover(self, *, dimension=None, require_verified=True):
        """Return certified regular CAD charts for intrinsic integration."""
        from .parametric_geometry import intrinsic_parametric_cover

        return intrinsic_parametric_cover(
            self, self.variables, dimension=dimension, require_verified=require_verified
        )
    ...
    def sample_point(self, *, strategy: str = "auto", **kwargs):
        """Return one certified representative point as a coordinate tuple."""

        from .sampling import sample_point

        point = sample_point(
            self.quantifier_free_formula(), self.variables, strategy=strategy, **kwargs
        )
        if point is None:
            return None
        return tuple(point[var] for var in self.variables)

    def sample_points(self, *, count: int = 1, strategy: str = "auto", **kwargs):
        """Return certified representative points as coordinate tuples."""
        ...
        return tuple(tuple(point[var] for var in self.variables) for point in points)
    ...
    def random_point(
        ...
        *,
        ...
        """Return a random point using bounded rejection sampling."""
    ...
    def random_points(
        ...
        *,
        ...
        """Return random points using bounded rejection sampling."""
    ...
    def dimension(self) -> int:
        return self.ensure_native_analysis().dimension()

    def is_bounded(self, *, strategy: str | None = None) -> bool:
        analysis = self.ensure_native_analysis()
        certified = analysis.bounded_certificate()
