...
def _aligned_z_parametric_cylinder(region: Cylinder, cone: bool = False) -> ParametricRegion | None:
    ...
    x1, y1, z1 = region.end
    if certified_equal(x1, x0) is not True or certified_equal(y1, y0) is not True:
        return None
    ...
    *,
    ...
    if isinstance(region, Interval):
        if region.dimension() < 0:
            return sp.Integer(0)
        if region.dimension() == 0:
            return sp.simplify(expr.subs(vars_[0], region.lower))
    ...
    if isinstance(region, Box):
        ...
        for variable, (lower, upper) in zip(vars_, region.bounds, strict=True):
            if certified_equal(upper, lower) is True:
                substitutions[variable] = lower
            else:
                active_variables.append(variable)
                active_bounds.append((lower, upper))
        reduced = sp.simplify(expr.subs(substitutions))
        if not active_variables:
            return reduced
        active_vars = tuple(active_variables)
        bounds = tuple(active_bounds)
        fast = _integrate_polynomial_over_box(reduced, active_vars, bounds)
        if fast is not None:
            return fast
        value = sp.integrate(
            reduced,
            *tuple(
                (v, lo, hi)
                for v, (lo, hi) in reversed(tuple(zip(active_vars, bounds, strict=True)))
            ),
        )
