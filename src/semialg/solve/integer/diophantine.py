...
def solve_integer_with_modular_pruning(
    ...
    *,
    ...
    The bounded lift is incomplete: modular inconsistency proves
...
def detect_sum_eqn(expr: sp.Expr, variables: Sequence[sp.Symbol]):
    ...
    allowed = {(2, 0), (0, 2), (0, 0)}
    if (
        set(poly.monoms()) <= allowed
        and poly.coeff_monomial(x**2) == 1
        and poly.coeff_monomial(y**2) == 1
    ):
        c = -poly.coeff_monomial(1)
        if c.is_integer is True and not c.free_symbols:
            return int(c)
    return None
