...
def detect_sum_fam2(expr: sp.Expr, variables: Sequence[sp.Symbol]) -> SpecialFamDesc | None:
    ...
    if cross_terms:
        return None
    if target.is_integer is True and not target.free_symbols:
...
def detect_diag_fam(expr: sp.Expr, variables: Sequence[sp.Symbol]) -> SpecialFamDesc | None:
    ...
    if not nonconst_terms:
        return None
    exponents = set()
    coefficient_by_variable: dict[int, int] = {}
    for mon, coeff in nonconst_terms:
        if coeff not in (1, -1):
            return None
        support = [index for index, exponent in enumerate(mon) if exponent != 0]
        if len(support) != 1:
            return None
        index = support[0]
        exponents.add(mon[index])
        coefficient_by_variable[index] = int(coeff)
    if len(exponents) != 1 or len(coefficient_by_variable) != len(variables):
        return None
    k = next(iter(exponents))
    target = -poly.coeff_monomial(1)
    if target.is_integer is True and not target.free_symbols:
        coefficients = tuple(coefficient_by_variable[index] for index in range(len(variables)))
        return SpecialFamDesc(
            "diagonal_sum_of_powers",
            variables,
            {"power": int(k), "target": int(target), "coefficients": coefficients},
        )
    return None
...
def detect_pell_family(expr: sp.Expr, variables: Sequence[sp.Symbol]) -> SpecialFamDesc | None:
    ...
    if (
        D.is_integer is True
        and N.is_integer is True
        and not D.free_symbols
        and not N.free_symbols
        and int(D) > 0
        and int(D) != isqrt(int(D)) ** 2
    ):
