...
def _prove_sign(
    ...
    *,
    ...
    if cheap is None and asm in (sp.true, True) and relation in {"nonnegative", "nonpositive"}:
        ...
        try:
            zeng = zeng_negative_point(target, vars_)
        except (RationalUnivariateError, sp.PolynomialError, ValueError, TypeError):
            zeng = None
        if zeng is not None and zeng.complete and zeng.has_negative_point is not None:
            cheap = (
                not zeng.has_negative_point,
                zeng.method,
                zeng.assignment if zeng.has_negative_point else zeng.notes,
            )
