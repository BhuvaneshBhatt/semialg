...
def _build_complete_qe_cad(
    matrix: Formula,
    variables: tuple[sp.Symbol, ...],
    quantifiers: tuple[tuple[str, sp.Symbol], ...],
    *,
    allow_variety_cad: bool,
) -> tuple[CompleteCAD, tuple[str, ...]]:
    """Construct the complete CAD backend without changing backend selection policy."""
    ...
    if groebner_projection is None:
    ...
    if variety_cad is None:
        ...
        return decomp_collins_complete(polys, variables), tuple(notes)
    notes.append("used zero-dimensional Groebner variety projection/lifting for existential CAD")
    return variety_cad, tuple(notes)


def _propagate_quantified_cell_truth(
    cad: CompleteCAD,
    matrix: Formula,
    variables: tuple[sp.Symbol, ...],
    free_level: int,
    qmap: Mapping[sp.Symbol, str],
) -> dict[tuple[int, ...], bool]:
    """Evaluate leaf cells and propagate truth through quantified CAD levels."""
    ...
    for level in range(len(variables), free_level, -1):
...
def _qe_by_complete_cad_result(
    ...
    *,
    ...
    cad, cad_notes = _build_complete_qe_cad(
        matrix, variables, quantifiers, allow_variety_cad=allow_variety_cad
    )
    note_list.extend(cad_notes)
    notes = tuple(note_list)
    free_level = len(free)
    current = _propagate_quantified_cell_truth(cad, matrix, variables, free_level, qmap)
