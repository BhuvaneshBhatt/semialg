...
from semialg.solve import InstanceResult, find_instance_text
from semialg.status import SolverStatus


def test_find_instance_01():
    x = sp.Symbol("x", real=True)
    result = find_instance(sp.Eq(x**2, 2), [x], count=2, return_result=True)
...
def test_find_instance_03():
    x = sp.Symbol("x")
    result = find_instance(sp.Eq(x**2 + 1, 0), [x], domain="complexes", count=2, return_result=True)
    assert isinstance(result, InstanceResult)
...
def test_find_instance_04():
    x, y = sp.symbols("x y")
    result = find_instance(x | y, [x, y], domain="booleans", count=2, return_result=True)
    assert len(result.instances) == 2
