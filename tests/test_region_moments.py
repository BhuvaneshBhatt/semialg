...
from semialg import centroid, covariance_matrix, region_moment
...
def test_centroid_and_covariance_on_symmetric_interval() -> None:
    ...
    center = centroid(x**2 <= 1, [x])
    assert center[x] == 0

    covariance = covariance_matrix(x**2 <= 1, [x])
...
def test_centroid_and_covariance_on_unit_square() -> None:
    ...
    center = centroid(square, [x, y])
    assert center == {x: sp.Rational(1, 2), y: sp.Rational(1, 2)}

    covariance = covariance_matrix(square, [x, y])
...
def test_centroid_on_standard_simplex() -> None:
    ...
    center = centroid(triangle, [x, y])
    assert center == {x: sp.Rational(1, 3), y: sp.Rational(1, 3)}

    covariance = covariance_matrix(triangle, [x, y])
...
def test_disk_moments_centroid_covariance() -> None:
    ...
    assert centroid(disk, [x, y]) == {x: 0, y: 0}
    assert covariance_matrix(disk, [x, y]) == sp.Matrix(
...
def test_intrinsic_circle_centroid_and_covariance() -> None:
    ...
    center = centroid(circle, [x, y], measure_dimension="intrinsic")
    assert center == {x: 0, y: 0}

    covariance = covariance_matrix(circle, [x, y], measure_dimension=1)
