...
from semialg.decomposition._parametric_support import ParametricCADSplit


def test_parametric_cad_nonparametric_disk_splits_interior_and_boundary():
    x, y = sp.symbols("x y", real=True)
    result = parametric_cad(x**2 + y**2 <= 1, [x, y])

    assert result.status == "complete"
    assert isinstance(result.generic_split, ParametricCADSplit)
...
def test_parametric_cad_keeps_deleted_boundary_exceptional():
    x = sp.symbols("x", real=True)
    result = parametric_cad(x**2 < 1, [x])
...
def test_parametric_cad_keeps_parameter_exceptional_cases():
