...
from semialg.decomposition import ParametricCADResult, component_instances, parametric_cad
...
def test_component_and_parametric_cad_defaults_are_mathematical_outputs():
    x = sp.Symbol("x", real=True)
    instances = component_instances(sp.Or(x < -1, x > 1), [x])
    assert isinstance(instances, tuple)
    assert len(instances) == 2
    result = parametric_cad(x**2 < 1, [x])
    assert isinstance(result, ParametricCADResult)
    assert result.generic_formula == (x**2 - 1 < 0)
