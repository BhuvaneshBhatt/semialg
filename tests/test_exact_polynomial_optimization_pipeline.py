...
def test_exact_pipeline_interior_stationary_point() -> None:
    ...
    result = semialgebraic_minimize(x**2 + 2 * y**2, [x**2 + y**2 <= 4], [x, y], return_result=True)
...
def test_exact_pipeline_multiple_active_boundaries_vertex() -> None:
    ...
    result = semialgebraic_minimize(
        x + 2 * y, [x >= 0, y >= 0, x <= 2, y <= 3], [x, y], return_result=True
    )
    assert result.value == 0
    assert result.point == {x: 0, y: 0}
    assert result.certified is True
...
def test_exact_pipeline_equality_constraint() -> None:
    x, y = sp.symbols("x y", real=True)
    result = semialgebraic_maximize(x, [sp.Eq(x**2 + y**2, 1)], [x, y], return_result=True)
...
def test_exact_pipeline_singular_active_locus() -> None:
    x, y = sp.symbols("x y", real=True)
    result = semialgebraic_minimize(x, [sp.Eq(x**2 + y**2, 0)], [x, y], return_result=True)
    assert result.value == 0
    assert result.point == {x: 0, y: 0}
    assert result.certified is True
...
def test_exact_pipeline_open_boundary_reports_unattained_infimum() -> None:
    x, y = sp.symbols("x y", real=True)
    result = semialgebraic_minimize(x, [x**2 + y**2 < 1], [x, y], return_result=True)
    assert result.value == -1
...
def test_exact_pipeline_three_variables() -> None:
    ...
    result = semialgebraic_maximize(
        x + y + z, [x**2 + y**2 + z**2 <= 1], [x, y, z], return_result=True
    )
...
def test_exact_pipeline_disjunctive_domain() -> None:
    ...
    result = semialgebraic_minimize(x**2, domain, [x], return_result=True)
    assert result.value == 1
...
def test_exact_pipeline_algebraic_optimum_and_rur_candidates() -> None:
    x, y = sp.symbols("x y", real=True)
    result = semialgebraic_minimize(x, [sp.Eq(x**2, 2), sp.Eq(y, 0)], [x, y], return_result=True)
...
def test_exact_pipeline_return_value_api() -> None:
    x, y = sp.symbols("x y", real=True)
    result = semialgebraic_maximize(x * y, [x >= 0, y >= 0, x + y <= 1], [x, y])
    assert result[0] == sp.Rational(1, 4)
    assert result[1] == [{x: sp.Rational(1, 2), y: sp.Rational(1, 2)}]
...
def test_disjunctive_optimization_requires_every_branch_to_be_certified() -> None:
    ...
    result = optimization_module._combine_branch_results(x, (x,), branch_results, kind="min")
