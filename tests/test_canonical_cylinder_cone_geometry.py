...
from semialg import AffineSpace, Cone, Cylinder, Line
...
def test_region_conversion_bridges_preserve_canonical_semantics():
    cylinder = Cylinder.from_region(Cylinder((0, 0, 0), (0, 0, 1), 3))
    cone = Cone.from_region(Cone((0, 0, 0), (0, 0, 1), 3))
    assert isinstance(cylinder, Cylinder)
    assert isinstance(cone, Cone)
    assert cylinder.radius == cone.radius == 3
    with pytest.raises(TypeError):
        Cylinder.from_region(Cone((0, 0, 0), (0, 0, 1), 1))
